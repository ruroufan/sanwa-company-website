      ...
      <div className="mx-auto max-w-7xl rounded-xl border border-warmLine/70 bg-mist px-5 py-9 md:rounded-2xl md:px-12 md:py-16">
        <div className="grid gap-8 md:grid-cols-[1fr_0.9fr] md:items-center md:gap-10">
          <div>
            ...
            <p className="mt-4 max-w-2xl text-[15px] leading-7 text-warmText md:mt-6 md:text-lg md:leading-8">
              {content.contact.description}
            </p>
            <a
              href={`mailto:${content.contact.email}`}
              className="mt-7 inline-flex h-11 w-full items-center justify-center rounded-full bg-sage px-7 text-sm font-medium text-white transition hover:bg-sageDark sm:w-auto md:mt-10 md:h-12"
            >
              {content.contact.cta}
            </a>
          </div>
          <div className="min-w-0 space-y-4 rounded-xl border border-warmLine/65 bg-ivory/72 p-5 text-sm leading-7 text-warmText md:p-6">
